...
static inline void __add_wait_queue(wait_queue_head_t *head, wait_queue_t *nnew)
{
#if WAITQUEUE_DEBUG
	if (!head || !nnew)
		WQ_BUG();
	CHECK_MAGIC_WQHEAD(head);
	CHECK_MAGIC(nnew->__magic);
	if (!head->task_list.next || !head->task_list.prev)
		WQ_BUG();
#endif
	list_add(&nnew->task_list, &head->task_list);
}
						...
						wait_queue_t *nnew)
{
	...
	if (!head->task_list.next || !head->task_list.prev)
		WQ_BUG();
#endif
	list_add_tail(&nnew->task_list, &head->task_list);
}
